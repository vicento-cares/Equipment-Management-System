...
if ($method == 'count_notif_sp') {
    ...
    $sql = "SELECT new_pm_concerns FROM t_notif_pm_no_spare WHERE interface = 'ADMIN-SP'";
    ...
    $sql = "SELECT pending_mstprc FROM t_notif_setup_approvers WHERE interface = 'APPROVER-2-EQ-SP'";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
...
if ($method == 'update_notif_new_pm_concerns') {
    $sql = "UPDATE t_notif_pm_no_spare SET new_pm_concerns = 0 WHERE interface = 'ADMIN-SP'";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
}

if ($method == 'update_notif_pending_mstprc') {
    $sql = "UPDATE t_notif_setup_approvers SET pending_mstprc = 0 WHERE interface = 'APPROVER-2-EQ-SP'";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
}
