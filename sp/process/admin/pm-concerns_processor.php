...
{
    $sql = "UPDATE t_machine_pm_concerns SET is_read_sp = 1 WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->execute([$id]);

    $sql = "UPDATE t_notif_pm_no_spare SET new_pm_concerns = CASE WHEN new_pm_concerns > 0 THEN new_pm_concerns - 1 END WHERE interface = 'ADMIN-SP'";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
}
...
if ($method == 'get_no_spare_pm_concerns') {
    ...
    $c = 0;
    $sql = "SELECT id, pm_concern_id, machine_line, machine_name, car_model, trd_no, [ns_iv_no], problem, request_by, confirm_by, comment, concern_date_time, no_of_parts, status, is_read_sp FROM t_machine_pm_concerns WHERE comment = 'NO SPARE' AND no_spare = 1 AND status = 'Pending' ORDER BY id DESC";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
...
if ($method == 'get_no_spare_by_pm_concerns_id_sp') {
    ...
    $c = 0;
    $sql = "SELECT id, pm_concern_id, machine_line, machine_name, car_model, trd_no, [ns_iv_no], problem, request_by, confirm_by, comment, concern_date_time, parts_code, quantity, po_date, po_no, no_spare_status, date_arrived, status 
...
if ($method == 'count_no_spare_history') {
    ...
    if ($pm_concern_status == 'Pending') {
        $sql = $sql . " FROM t_machine_pm_no_spare";
    } else if ($pm_concern_status == 'Done') {
        $sql = $sql . " FROM t_machine_pm_no_spare_history";
    }
    ...
    if (!empty($machine_name) || !empty($car_model) || !empty($pm_concern_id) || (!empty($concern_date_from) && !empty($concern_date_to))) {
        $sql = $sql . " WHERE machine_name LIKE ? AND car_model LIKE ? AND 
                        ...
                        concern_date_time <= ?) AND no_spare_status = 'CLOSE'";
        $params = [
            $machine_name . "%",
            $car_model . "%",
            $pm_concern_id . "%",
...
if ($method == 'get_no_spare_history') {
    ...
    $sql = "SELECT TOP 25 id, pm_concern_id, machine_line, machine_name, car_model, trd_no, [ns_iv_no], problem, request_by, confirm_by, comment, concern_date_time, parts_code, quantity, po_date, po_no, no_spare_status, date_arrived, status, date_updated";
    $params = [];
    ...
    if (empty($id)) {
        if (!empty($machine_name) || !empty($car_model) || !empty($pm_concern_id) || (!empty($concern_date_from) && !empty($concern_date_to))) {
            $sql = $sql . " WHERE machine_name LIKE ? AND car_model LIKE ? AND 
                            pm_concern_id LIKE ? AND (concern_date_time >= ? AND concern_date_time <= ?)";
            $params = [
                $machine_name . "%",
                $car_model . "%",
                $pm_concern_id . "%",
