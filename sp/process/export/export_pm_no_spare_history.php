...
$sql = "SELECT id, pm_concern_id, machine_line, machine_name, car_model, trd_no, [ns_iv_no], problem, request_by, confirm_by, comment, concern_date_time, parts_code, quantity, po_date, po_no, no_spare_status, date_arrived, status, date_updated";
$params = [];
...
while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    $lineData = array($c, $date_updated, $row['pm_concern_id'], $row['machine_line'], $row['machine_name'], $row['car_model'], $row['trd_no'], $row['ns_iv_no'], $row['problem'], $row['request_by'], $row['confirm_by'], $row['comment'], $concern_date_time, $row['parts_code'], $row['quantity'], $po_date, $row['po_no'], $row['no_spare_status'], $date_arrived, $row['status']);
