...
document.addEventListener("DOMContentLoaded", () => {
  ...
  } else if (getCookie('setup_role') == 'Admin' && getCookie('setup_process') == 'Final') {
    document.getElementById("cardAddMachine").style.display = 'none';
    document.getElementById("cardExportMachineFormat").style.display = 'none';
    document.getElementById("cardUploadMachine").style.display = 'none';
...
$("#UploadUnusedMachineMasterlistCsvModal").on('hidden.bs.modal', e => {
  document.getElementById('unused_machine_masterlist_file').value = '';
});

const upload_unused_machine_masterlist_csv = () => {
  var form_data = new FormData();
  var ins = document.getElementById('unused_machine_masterlist_file').files.length;
  for (var x = 0; x < ins; x++) {
    form_data.append("file", document.getElementById('unused_machine_masterlist_file').files[x]);
  }
  $.ajax({
    url: '../process/import/import_unused_machine_masterlist.php',
    type: 'POST',
    dataType: 'text',
    cache: false,
    contentType: false,
    ...
    success: response => {
      setTimeout(() => {
        ...
        if (response != '') {
          ...
          swal('Upload CSV', 'Uploaded and updated successfully', 'success');
          load_data(1);
          $('#UploadUnusedMachineMasterlistCsvModal').modal('hide');
        }
      }, 500);
    }
  })
