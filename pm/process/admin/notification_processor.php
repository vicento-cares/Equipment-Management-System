...
if ($method == 'count_notif_pm') {
    ...
    $sql = "SELECT new_pm_concerns FROM t_notif_pm_concerns WHERE interface = 'ADMIN-PM'";
    ...
    $sql = "SELECT approved_rsir FROM t_notif_pm_approvers WHERE interface = 'ADMIN-PM'";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
    ...
    $sql = "SELECT disapproved_rsir FROM t_notif_pm_approvers WHERE interface = 'ADMIN-PM'";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
...
if ($method == 'count_notif_pending_rsir') {
    ...
    if (!empty($interface)) {
        $sql = "SELECT pending_rsir FROM t_notif_pm_approvers WHERE interface = ?";
        $stmt = $conn->prepare($sql);
...
if ($method == 'count_notif_public_pm_concerns') {
    ...
    $total = 0;
    $sql = "SELECT done_pm_concerns, pending_pm_concerns FROM t_notif_pm_concerns WHERE interface = 'PUBLIC-PAGE'";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
...
if ($method == 'update_notif_new_pm_concerns') {
    $sql = "UPDATE t_notif_pm_concerns SET new_pm_concerns = 0 WHERE interface = 'ADMIN-PM'";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
}

if ($method == 'update_notif_approved_rsir') {
    $sql = "UPDATE t_notif_pm_approvers SET approved_rsir = 0 WHERE interface = 'ADMIN-PM'";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
}

if ($method == 'update_notif_disapproved_rsir') {
    $sql = "UPDATE t_notif_pm_approvers SET disapproved_rsir = 0 WHERE interface = 'ADMIN-PM'";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
}
...
if ($method == 'update_notif_pending_rsir') {
    ...
    if (!empty($interface)) {
        $sql = "UPDATE t_notif_pm_approvers SET pending_rsir = 0 WHERE interface = ?";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$interface]);
    }
}
...
if ($method == 'update_notif_public_pm_concerns') {
    $sql = "UPDATE t_notif_pm_concerns SET done_pm_concerns = 0, pending_pm_concerns = 0 WHERE interface = 'PUBLIC-PAGE'";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
}
