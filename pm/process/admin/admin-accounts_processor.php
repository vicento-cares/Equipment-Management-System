...
{
    $sql = "SELECT username FROM m_machine_pm_accounts WHERE username = ?";
...
{
    $sql = "SELECT username FROM m_machine_pm_accounts WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->execute([$id]);
...
function change_username($id, $username, $date_updated, $conn)
{
    ...
    if (!$row) {
        $sql = "UPDATE m_machine_pm_accounts SET username = ?, date_updated = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
...
if ($method == 'get_accounts_pm_dropdown') {
    $sql = "SELECT name, process FROM m_machine_pm_accounts WHERE role = 'PM' ORDER BY name ASC";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
...
if ($method == 'count_data') {
    $search = $_POST['search'];
    $sql = "SELECT COUNT(id) AS total FROM m_machine_pm_accounts";
    $params = [];
    if (!empty($search)) {
...
if ($method == 'fetch_data') {
    ...
    $sql = "SELECT TOP 10 id, username, password, name, role, process, date_updated FROM m_machine_pm_accounts";
    $params = [];
    ...
    if ($is_valid == true) {
        if ($own_role == 'Admin') {
            ...
            if ($is_existing == false) {
                $sql = "INSERT INTO m_machine_pm_accounts (username, password, name, role, process, date_updated) 
                        VALUES (?, ?, ?, ?, ?, ?)";
...
if ($method == 'update_password') {
    ...
    if ($is_valid_password == true) {
        ...
        if ($own_role != 'Admin' && $is_own_username == false) {
            ...
            $sql = "UPDATE m_machine_pm_accounts SET password = ?, date_updated = ? WHERE id = ?";
            $stmt = $conn->prepare($sql);
    ...
    if ($is_valid == true) {
        ...
        if ($own_role == 'Admin') {
            if ($is_own_username == true && $role != 'Admin') {
                ...
                $sql = "UPDATE m_machine_pm_accounts SET name = ?, role = ?, process = ?, date_updated = ? WHERE id = ?";
                $stmt = $conn->prepare($sql);
            ...
            if ($role == 'Admin') {
                ...
                $sql = "UPDATE m_machine_pm_accounts SET name = ?, date_updated = ? WHERE id = ?";
                $stmt = $conn->prepare($sql);
...
if ($method == 'delete_data') {
    ...
    if ($is_own_username == false) {
        if ($own_role == 'Admin') {
            $sql = "DELETE FROM m_machine_pm_accounts WHERE id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->execute([$id]);
            echo 'success';
        } else {
