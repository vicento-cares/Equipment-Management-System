...
function check_existing_username($username, $conn) {
	...
	$sql = "SELECT `username` FROM `machine_pm_accounts` WHERE username = '$username'";
...
function check_own_username($id, $own_username, $conn) {
	$sql = "SELECT `username` FROM `machine_pm_accounts` WHERE id = '$id'";
	$stmt = $conn -> prepare($sql);
	$stmt -> execute();
...
function change_username($id, $username, $date_updated, $conn) {
	...
	$stmt -> execute();
	if ($stmt -> rowCount() <= 0) {
		$sql = "UPDATE `machine_pm_accounts` SET `username`= '$username', `date_updated`= '$date_updated' WHERE id = '$id'";
		$stmt = $conn -> prepare($sql);
		$stmt -> execute();
		return true;
	} else {
...
if ($method == 'get_accounts_pm_dropdown') {
	$sql = "SELECT `name`, `process` FROM `machine_pm_accounts` WHERE `role`= 'PM' ORDER BY `name` ASC";
	$stmt = $conn -> prepare($sql);
	$stmt -> execute();
	if ($stmt -> rowCount() > 0) {
		echo '<option disabled selected value="">Select PM Account</option>';
...
if ($method == 'count_data') {
	...
	$sql = "SELECT count(id) AS total FROM `machine_pm_accounts`";
	if (!empty($search)) {
...
if ($method == 'fetch_data') {
	...
	$search = addslashes($_POST['search']);
	$c = $_POST['c'];
	$own_username = $_SESSION['pm_username'];
	$own_role = $_COOKIE['pm_role'];
	$own_name = $_COOKIE['pm_name'];
	$row_class_arr = array('modal-trigger', 'modal-trigger bg-lime');
	$row_class = $row_class_arr[0];
	$sql = "SELECT `id`, `username`, `password`, `name`, `role`, `process`, `date_updated` FROM `machine_pm_accounts`";
	...
	if ($is_valid_username == true) {
		if ($is_valid_password == true) {
			if ($is_valid_name == true) {
				if ($role != '') {
					if ($process != '') {
						if ($process != 'N/A' && ($role == 'Admin' || $role == 'PM')) {
							$is_valid = true;
						} else echo 'Process Not Allowed';
	...
	if ($is_valid == true) {
		if ($own_role == 'Admin') {
			...
			if ($is_existing == false) {
				...
				$sql = "INSERT INTO `machine_pm_accounts` (`username`, `password`, `name`, `role`, `process`, `date_updated`) VALUES ('$username', '$password', '$name', '$role', '$process', '$date_updated')";
				$stmt = $conn -> prepare($sql);
				$stmt -> execute();
				echo 'success';
			} else {
...
if ($method == 'update_password') {
	...
	if ($is_valid_password == true) {
		...
		if ($own_role != 'Admin' && $is_own_username == false) {
			...
			$password = addslashes($password);
			$sql = "UPDATE `machine_pm_accounts` SET `password`= '$password', `date_updated`= '$date_updated' WHERE id = '$id'";
			$stmt = $conn -> prepare($sql);
			$stmt -> execute();
			echo 'success';
		}
	...
	if ($is_valid == true) {
		...
		if ($own_role == 'Admin') {
			if ($is_own_username == true && $role != 'Admin') {
				...
				$name = addslashes($name);
				$sql = "UPDATE `machine_pm_accounts` SET `name`= '$name', `role`= '$role', `process`= '$process', `date_updated`= '$date_updated' WHERE `id`= '$id'";
				$stmt = $conn -> prepare($sql);
				$stmt -> execute();
				if ($is_own_username == true) {
					setcookie('pm_name', $name, 0, "/ems");
					$_SESSION['pm_name'] = $name;
				}
				echo 'success';
			}
		} else {
			if ($role == 'Admin') {
				...
				$name = addslashes($name);
				$sql = "UPDATE `machine_pm_accounts` SET `name`= '$name', `date_updated`= '$date_updated' WHERE `id`= '$id'";
				$stmt = $conn -> prepare($sql);
				$stmt -> execute();
				if ($is_own_username == true) {
					setcookie('pm_name', $name, 0, "/ems");
...
if ($method == 'delete_data') {
	...
	if ($is_own_username == false) {
		if ($own_role == 'Admin') {
			$sql = "DELETE FROM `machine_pm_accounts` WHERE id = '$id'";
			$stmt = $conn -> prepare($sql);
			$stmt -> execute();
			echo 'success';
		} else {
