...
function check_machine_docs_file($machine_docs_file_info, $action, $conn)
{
    ...
    $sql = "SELECT id FROM t_machine_pm_docs WHERE file_name = ? AND file_type = ? AND file_url = ?";
...
function save_machine_docs_info($machine_docs_file_info, $conn)
{
    ...
    $sql = "INSERT INTO t_machine_pm_docs (process, machine_name, machine_docs_type, file_name, file_type, file_url, date_updated) 
            VALUES (?, ?, ?, ?, ?, ?, ?)";
...
function update_machine_docs_info($machine_docs_file_info, $conn)
{
    ...
    $sql = "UPDATE t_machine_pm_docs 
            SET process = ?, machine_name = ?, machine_docs_type = ?, 
            file_name = ?, file_type = ?, file_url = ?, date_updated = ? 
            WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->execute([$process, $machine_name, $machine_docs_type, $machine_docs_filename, $machine_docs_filetype, $machine_docs_url, $date_updated, $id]);
}

// Count
if ($method == 'count_machine_docs') {
    $search = $_POST['search'];
    $sql = "SELECT COUNT(id) AS total FROM t_machine_pm_docs";
    $params = [];
    if (!empty($search)) {
...
if ($method == 'load_machine_docs') {
    ...
    $sql = "SELECT TOP 10 id, process, machine_name, machine_docs_type, file_name, file_url, date_updated FROM t_machine_pm_docs";
...
if ($method == 'update_machine_docs') {
    ...
    if ($is_valid == true) {
        ...
        if (!empty($_FILES['file']['name'])) {
            ...
            $sql = "SELECT file_name FROM t_machine_pm_docs WHERE id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->execute([$id]);
            ...
            $sql = "UPDATE t_machine_pm_docs 
                    SET process = ?, machine_name = ?, machine_docs_type = ?, date_updated = ? 
                    WHERE id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->execute([$process, $machine_name, $machine_docs_type, $date_updated]);
        }
    }
}
...
if ($method == 'delete_machine_docs') {
    ...
    $sql = "SELECT machine_docs_type, file_name FROM t_machine_pm_docs WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->execute([$id]);
    ...
    if (file_exists($target_file)) {
        if (unlink($target_file)) {
            $sql = "DELETE FROM t_machine_pm_docs WHERE id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->execute([$id]);
        } else {
