...
function get_current_number_by_name($machine_name, $conn)
{
    ...
    $sql = "SELECT TOP 1 number FROM m_machines WHERE machine_name = '$machine_name' ORDER BY number DESC";
...
$start_row = 1;
$insertsql = "INSERT INTO m_machine_masterlist (number, process, machine_name, machine_spec, car_model, location, grid, machine_no, equipment_no, asset_tag_no, trd_no, [ns_iv_no], is_new, date_updated) VALUES ";
$subsql = "";
    ...
    if ($first_line == $valid_first_line1 || $first_line == $valid_first_line2) {
        while (($line = fgetcsv($csvFile)) !== false) {
            ...
            if ($car_model_raw != 'EQ-Initial' && $car_model_raw != 'EQ-Final') {
                $hasError = 1;
                $row_valid_arr[8] = 1;
                array_push($notValidCarModelArr, $check_csv_row);
            }
            if ($location != 'FAS4') {
                $hasError = 1;
                $row_valid_arr[9] = 1;
            ...
            $sql = "SELECT id FROM m_machine_masterlist 
                        WHERE process = ? AND machine_name = ? AND machine_spec = ? AND 
                            car_model = ? AND location = ? AND grid = ? AND machine_no = ? AND 
                            equipment_no = ? AND asset_tag_no = ? AND trd_no = ? AND [ns_iv_no] = ?";
            $stmt = $conn->prepare($sql);
            $stmt->execute([
                $process, $machine_name, $machine_spec, 
                $car_model, $location, $grid, $machine_no, 
                $equipment_no, $asset_tag_no, $trd_no, $ns_iv_no
            ]);
