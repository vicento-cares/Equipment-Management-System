...
function get_machine_details_by_id($machine_no, $equipment_no, $conn)
{
    ...
    $sql = "SELECT number, process, machine_name, machine_spec, car_model, location, grid, machine_no, equipment_no, asset_tag_no, trd_no, [ns_iv_no], machine_status, is_new 
...
{
    $sql = "SELECT id FROM t_pm_rsir 
            WHERE machine_no = ? AND equipment_no = ? AND rsir_process_status != 'Returned' AND rsir_eq_group = ?";
    $stmt = $conn->prepare($sql);
