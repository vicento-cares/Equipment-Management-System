...
$sql = "SELECT 
            id, number, process, machine_name, machine_spec, car_model, location, grid, machine_no, equipment_no, trd_no, [ns_iv_no], pm_plan_year, ww_no, ww_start_date, frequency, machine_status, pm_status 
        FROM t_machine_pm_plan 
        WHERE pm_plan_year LIKE ?";
...
while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    $lineData = array($row['number'], $row['process'], $row['machine_name'], $row['machine_spec'], $row['car_model'], $row['location'], $row['grid'], $machine_no, $equipment_no, $row['trd_no'], $row['ns_iv_no'], $row['pm_plan_year'], $row['ww_no'], $row['ww_start_date'], $row['frequency'], $row['machine_status'], $row['pm_status']);
