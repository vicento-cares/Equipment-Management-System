  <meta name="description" content="EEMS PM" />
  <meta name="keywords" content="EEMS, PM, Machine" />

  <title>EEMS PM | Machine Checksheets</title>

  <!-- Google Font: Source Sans Pro -->
  <link rel="preload" href="../../dist/css/font.min.css" as="style" onload="this.onload=null;this.rel='stylesheet'">
  <!-- Font Awesome -->
  <link rel="preload" href="../../plugins/fontawesome-free/css/all.min.css" as="style" onload="this.onload=null;this.rel='stylesheet'">
  <!-- Theme style -->
  <link rel="preload" href="../../dist/css/adminlte.min.css" as="style" onload="this.rel='stylesheet'">
  <link rel="stylesheet" href="../../dist/css/adminlte.min.css">
  <!-- Ionicons -->
  <link rel="preload" href="../../plugins/ionicons/css/ionicons.min.css" as="style" onload="this.onload=null;this.rel='stylesheet'">
  <!-- overlayScrollbars -->
  <link rel="preload" href="../../plugins/overlayScrollbars/css/OverlayScrollbars.min.css" as="style" onload="this.onload=null;this.rel='stylesheet'">

  <style type="text/css">
    .preloader {
      width: 100%;
      background-image: url('../../dist/img/ems-bg-img-pm.webp');
      background-size: cover;
    }
    .dark-mode a:not(.btn):hover {
      color: #01ff70;
    }
    /* CUSTOM SWEETALERT COLOR */
    .swal-icon--success:before {
      background-color: rgba(52, 58, 64, 1);
    }
    .swal-icon--success__hide-corners {
      background-color: rgba(52, 58, 64, 1);
    }
    .swal-icon--success:after {
      background-color: rgba(52, 58, 64, 1);
    }
    .swal-modal {
      background-color: rgba(52, 58, 64, 1);
    }
    .swal-title {
      color: rgba(255, 255, 255, 1);
    }
    .swal-text {
      color: rgba(255, 255, 255, 1);
    }
    .swal-button {
      background: #01ff70;
      color: #343a40;
    }
    .swal-button:not([disabled]):hover {
      background-color: #00cd5a;
    }
  </style>

  <noscript>
    <link rel="stylesheet" href="../../dist/css/font.min.css">
    <link rel="stylesheet" href="../../plugins/fontawesome-free/css/all.min.css">
    <link rel="stylesheet" href="../../dist/css/adminlte.min.css">
    <link rel="stylesheet" href="../../plugins/ionicons/css/ionicons.min.css">
    <link rel="stylesheet" href="../../plugins/overlayScrollbars/css/OverlayScrollbars.min.css">
  </noscript>

  <link rel="icon" type="image/x-icon" href="../../dist/img/ems-logo.ico">
</head>