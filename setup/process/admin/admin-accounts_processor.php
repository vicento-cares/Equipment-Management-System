...
if ($method == 'fetch_data') {
	...
	if ($stmt -> rowCount() > 0) {
		foreach($stmt -> fetchAll() as $row) {
			...
			echo '<tr style="cursor:pointer;" class="'.$row_class.'" id="'.$row['id'].'" data-toggle="modal" data-target="#AccountInfoModal" data-id="'.$row['id'].'" data-username="'.htmlspecialchars($row['username']).'" data-name="'.htmlspecialchars($row['name']).'" data-role="'.$row['role'].'" data-approver_role="'.$row['approver_role'].'" data-process="'.$row['process'].'" data-date_updated="'.$row['date_updated'].'" onclick="get_details(this)">';
			echo '<td>'.$c.'</td>';
...
if ($method == 'save_data') {
	...
	$role = $_POST['role'];
	$approver_role = $_POST['approver_role'];
		...
		if ($is_valid_password == true) {
			if ($is_valid_name == true) {
				if ($role != '') {
					if ($approver_role != '') {
						if ($process != '') {
							if ($process != 'N/A' && ($role == 'Admin' || $role == 'Setup')) {
								$is_valid = true;
							} else echo 'Process Not Allowed';
						} else echo 'Process Not Set';
					} else echo 'Approver Role Not Set';
