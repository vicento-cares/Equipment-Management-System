...
if ($method == 'fetch_data') {
    ...
    $sql = "SELECT TOP 25 id, number, process, machine_name, machine_spec, car_model, location, grid, machine_no, equipment_no, asset_tag_no, trd_no, [ns_iv_no], machine_status, is_new, date_updated FROM m_machine_masterlist";
    $params = [];
    ...
    if (empty($id)) {
        ...
        $params[] = $id;
        if (!empty($car_model) || !empty($machine_spec) || !empty($machine_name) || !empty($machine_no) || !empty($equipment_no)) {
            $sql = $sql . " AND (car_model LIKE ? AND machine_spec LIKE ? AND 
                            machine_name LIKE ? AND machine_no LIKE ? AND equipment_no LIKE ?";
            $params[] = $car_model . "%";
            $params[] = $machine_spec . "%";
            $params[] = $machine_name . "%";
...
if ($method == 'save_data') {
    ...
    if ($is_valid == true) {
        ...
        if ($is_exists_arr['machine_no_exists'] == true) {
            ...
            $sql = "INSERT INTO m_machine_masterlist (number, process, machine_name, machine_spec, car_model, location, grid, machine_no, equipment_no, asset_tag_no, trd_no, ns_iv_no, is_new, date_updated) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
